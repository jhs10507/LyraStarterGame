...
class AActor;						// 언리얼 엑터
class AController;                  // 언리얼 컨트롤러
class ALyraPlayerController;        // 라이라 캐릭터 컨트롤러
class ALyraPlayerState;             // 라이라 플레이어 상태
class FLifetimeProperty;            // 실시간 정보
class IRepChangedPropertyTracker;   // 
class UAbilitySystemComponent;      // 언리얼 능력치 시스템 컴포넌트
class UInputComponent;               // 언리얼 입력 컨포넌트
class ULyraAbilitySystemComponent;   // 라이라 어빌리티 시스템 컴포넌트
class ULyraCameraComponent;          // 라이라 카메라 컴포넌트
class ULyraHealthComponent;          // 라이라 체력 컴포넌트
class ULyraPawnExtensionComponent;   // 라이라 폰 확장 컴포넌트
class UObject;                       // 언리얼 오브젝트
struct FFrame;                       // 프레임
struct FGameplayTag;                 // 게임 플레이 테그
struct FGameplayTagContainer;        // 게임 플레이 테그 컨테이너
